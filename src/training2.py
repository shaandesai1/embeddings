    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train','validation']:
            ...
            if phase == 'train':
                ...
                writer.add_embedding(avg_class_iou,global_step=epoch,tag='miou_class_train')
            else:
                writer.add_scalar('sem_iou_val_epoch',avg_iou,epoch)
                writer.add_scalar('Loss_val_epoch',avg_loss,epoch)
                writer.add_embedding(avg_class_iou,global_step=epoch,tag='miou_class_val')
                #   scheduler.step(loss)
                if avg_iou > best_iou:
                    best_iou = avg_iou
                    print('Best Model!')
